...
def generate_launch_description():
    iiwa_controller_file = f"/ros2_dev/iiwa_ros2/install/iiwa_description/share/iiwa_description/config/iiwa_controllers.yaml"

    shutil.copyfile("/ros_app/iiwa_controllers.yaml", iiwa_controller_file)
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "robot_controller",
            default_value="external_controller",
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value="/ros_app/initial_positions.yaml",
    ...
    use_sim = "true"  # LaunchConfiguration("use_sim")
    use_fake_hardware = "true"  # LaunchConfiguration("use_fake_hardware")
    ...
    iiwa_planning_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [ThisLaunchFileDir(), "/iiwa_planning.launch.py"]
        ),
        launch_arguments={
    ...
    iiwa_servoing_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [ThisLaunchFileDir(), "/iiwa_servoing.launch.py"]
        ),
        launch_arguments={
            "description_package": description_package,
            "description_file": description_file,
    ...
    iiwa_simulation_world = PathJoinSubstitution(
        [FindPackageShare(description_package), "gazebo/worlds", "empty.world"]
    )
